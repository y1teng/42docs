...
このファイルは、循環型配列の動きを確認するための実験用コード。

## 使い方

```bash
cc -Wall -Wextra -Werror queue_debug.c && ./a.out
```
...
`buffer` の未使用部分をあらかじめ `'_'` で埋めているので、`print_queue()` で表示してもゴミが出ない。

42 Norm を意識するので、`for` は使わず `while` で書く。
